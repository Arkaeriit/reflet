...
* The bits 8 and up are the reduced behaviors bits. When they are all set to 0, the processor behaves normally. When they are set to `bxxxx1`, if the word size of the processor is above 8 bits, the processor will act as a 8-bit processor when interfacing with memory. When they are set to `bxxxx10`, if the word size of the processor is above 16 bits, the processor will act as a 16-bit processor when interfacing with memory. To summarize, The processor will act as a `8 * (2 ^ (reduced-behavior - 1))` bits processor when interacting with memory. Those bits are not taken into account when interacting with the stack such as with `pop` and `push` instructions.
